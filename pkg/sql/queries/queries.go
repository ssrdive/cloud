...
const SALE_ALL = `
	SELECT S.id, COUNT(SC.sale_id) as sale_comments,  R.name as region_name, S.date, S.sys_date,  S.chassis_no, S.customer_name, S.customer_address, S.customer_contact,  S.invoice_no, S.price,  S.institute, S.advance, S.deleted, M.name as model_name
	FROM sale S
...
const REGION_DATE_RANGEALL = `
	SELECT S.id, S.date, S.sys_date,  R.name as region_name, S.deleted
	FROM sale S
	LEFT JOIN region R ON S.region = R.id 
...
const DATE_RANGEALL = `
	SELECT S.id, S.date, S.sys_date, S.deleted
	FROM sale S
	WHERE  S.deleted = 0 AND (? IS NULL OR CONCAT(S.id) LIKE ?)
`
const SEARCH_DATE_RANGEALL = `
	SELECT S.id, S.date, S.sys_date, S.deleted
	FROM sale S
